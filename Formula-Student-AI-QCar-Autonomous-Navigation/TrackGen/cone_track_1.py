...
def generate_spline_closed_loop(track_width=4.8, spacing=2.0):
    """
    Generate left and right cone coordinates along a closed-loop spline path.
    """
    # Control points for a basic loop
    ctrl_x = [0, 10, 20, 25, 20, 10, 0, -10, -20, -25, -20, -10]
    ctrl_y = [0, 5, 0, -10, -20, -25, -20, -25, -20, -10, 0, 5]

    # Fit closed-loop spline
    tck, _ = splprep([ctrl_x, ctrl_y], s=0, per=True)

    # Uniform sampling
    num_points = int(1 / spacing * 200)
...
def setup_with_cones_demo(
    ...
    hqcar.possess()
    print("✅ Track setup complete: Closed-loop racing circuit ready.")
