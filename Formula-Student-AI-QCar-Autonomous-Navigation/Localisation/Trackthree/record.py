...
class QCarStraightPathRecorder:
    def __init__(self, model_path, save_path=r"C:\Users\Joshv\Desktop\Presentation\Code\Spawn&Navigation\Path\StraightTrack"):
        ...
        # Manual path recording variables
    ...
    def initialize_qlabs_qcar(self, qlabs):
        ...
        print("QLabs QCar initialized for position tracking")
    ...
    def save_path_manually(self):
        """Save current path data manually"""
        if len(self.path_data) > 0:
            try:
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                filename = f"straight_path_{self.recording_number}_{timestamp}.json"
                ...
                print(f"Points: {len(self.path_data)}")
                
                # Clear path data and increment recording number
                self.path_data = []
                self.recording_number += 1
    ...
    def calculate_lane_center(self, yellow_cones, blue_cones):
        ...
        if yellow_target and blue_target:
            ...
            blue_x = blue_target['center'][0]
            lane_center = blue_x + self.inner_boundary_offset
            confidence = 0.5
            
        elif yellow_target:
            yellow_x = yellow_target['center'][0]
            lane_center = yellow_x - self.outer_boundary_offset
            confidence = 0.4
    ...
    def calculate_steering_command(self, yellow_cones, blue_cones):
        ...
        # Speed control based on cone proximity
    ...
    def draw_detections(self, frame, yellow_cones, blue_cones):
        ...
        # Status information
        cv2.putText(frame_copy, f"Recording: {self.recording_number}", 
                   (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        
        status_text = f"Speed: {self.current_speed:.3f} | Steering: {self.current_steering:.2f}"
        cv2.putText(frame_copy, status_text, (10, self.img_height-60), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
        
        path_info = f"Path points: {len(self.path_data)}"
        cv2.putText(frame_copy, path_info, (10, self.img_height-35), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        
        # Instructions
        cv2.putText(frame_copy, "Press 'p' to save path | 'q' to quit | 's' to stop", 
                   (10, self.img_height-10), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)
    ...
    def run_navigation(self, qlabs, show_video=True, max_runtime=600):
        ...
        print("STRAIGHT TRACK PATH RECORDER STARTED")
        print("Controls:")
        print("  'p' - Save current path")
        print("  's' - Stop car")
        print("  'q' - Quit")
                    ...
                    while self.running and (time.time() - start_time) < max_runtime:
                        ...
                        if front_frame is not None:
                            ...
                            if show_video:
                                ...
                                cv2.imshow('QCar Straight Path Recorder', display_frame)
                                ...
                                if key == ord('q'):
                                    ...
                                    if self.save_path_manually():
                                        print("Path saved! Ready for next recording.")
...
def setup_qlabs_connection():
    ...
    hSystem.set_title_string('QCar Straight Path Recorder')
    
    return qlabs

def main():
    MODEL_PATH = r"C:\Users\Joshv\Desktop\QCar\Datasets\runs\detect\combined_cones\weights\best.pt"
    SAVE_PATH = r"C:\Users\Joshv\Desktop\Presentation\Code\Spawn&Navigation\Path\Trackthree"
    
    try:
        qlabs = setup_qlabs_connection()
        time.sleep(2)
        
        recorder = QCarStraightPathRecorder(MODEL_PATH, SAVE_PATH)
        recorder.run_navigation(qlabs, show_video=True, max_runtime=600)
